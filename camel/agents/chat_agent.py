...
class ChatAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.model_config: GeminiConfig = GeminiConfig() if self.model in {ModelType.GEMINI_PRO} else model_config or ChatGPTConfig()
    ...
    def step(
        ...
        if num_tokens < self.model_token_limit:
            if self.model in {
                ...
                output_messages = [
                        ChatMessage(role_name=self.role_name, role_type=self.role_type,
                                    meta_dict=dict(), content= choice.content.text, role='user' if choice.content.role == 'user' else 'assistant')
