...
class GeminiModel(ModelBackend):
    ...
    def run(self, *args, **kwargs):
        vertexai.init()
        string = "\n".join([message["content"] for message in kwargs["messages"]])
        model = GenerativeModel(self.model_type.value)
        ...
        response = model.generate_content(*args, contents=messages, generation_config=generation_config)

        cost = prompt_cost(
                self.model_type.value,
                num_prompt_tokens=response._raw_response.usage_metadata.prompt_token_count,
                num_completion_tokens=response._raw_response.usage_metadata.candidates_token_count
            )

        log_visualize(
            "**[Gemini_Usage_Info Receive]**\nprompt_tokens: {}\ncompletion_tokens: {}\ntotal_tokens: {}\ncost: ${:.6f}\n".format(
                response._raw_response.usage_metadata.prompt_token_count,response._raw_response.usage_metadata.candidates_token_count,
                response._raw_response.usage_metadata.total_token_count, cost))
